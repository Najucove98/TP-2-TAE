...
```{r}
# Esta opcion la descartamos por el grafico de georeferencia
...
cookies_dispositivos_diarios <- cookies_fecha |> 
  ...
  summarise(gasto_dia = sum(gasto, na.rm = TRUE),
    .groups = "drop") #creo que es como el ungroup
...
grafico <- ggplot(data = usuarios[1:100, ]) +
  aes(x = visitas, 
      y = gasto,
      text = paste("Tiempo Total:", round(tiempo, 1), "<br>",
                   "Páginas Total:", paginas, "<br>",
                   "Clicks Total:", clicks)) +
  geom_point() +
  labs(title = "Relación Visitas vs. Tiempo por Usuario")
...
```{r}
grafico <- ggplot(data = datos_visita_unica) +
  aes(x = n_visitas_efectivas, 
      y = gasto_promedio,
      text = paste0(
    "<b>Visitas efectivas: </b>", n_visitas_efectivas, "<br>",
    "<b>Gasto promedio: </b>", round(gasto_promedio, 2), " USD <br>",
    "<b>Tiempo promedio: </b>", round(tiempo_promedio, 2), " min <br>",
    "<b>Clicks promedio: </b>", round(clicks_promedio), "<br>",
    "<b>Paginas promedio: </b>", round(paginas_promedio))) +
  geom_point() +
...
```{r}
#Esta parte del codigo es solo para ver como comentarlo no deberia correr
